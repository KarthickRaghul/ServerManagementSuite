...
const AlertDashboard: React.FC = () => {
  ...
  useEffect(() => {
    if (filteredAlerts.length === 0) {
      ...
      const allSelected = filteredAlerts.every((alert) =>
  ...
  }, [filteredAlerts.length, selectedAlerts.length, selectAll]);

  // ✅ Updated refresh handler to use hook's refresh function
  ...
  // ✅ Updated bulk acknowledge to remove duplicate notifications
  ...
  }, [selectedAlerts, markSingleAlertAsSeen]);

  // ✅ Updated bulk resolve to remove duplicate notifications
  const handleBulkResolve = useCallback(async () => {
    ...
    if (
      window.confirm(
        `Are you sure you want to resolve ${selectedAlerts.length} selected alert(s)? This action cannot be undone.`,
      )
    ) {
      try {
  ...
  // ✅ Updated single acknowledge to remove duplicate notifications
  const handleSingleAcknowledge = useCallback(
    async (alertId: number) => {
      try {
        await markSingleAlertAsSeen(alertId);
      } catch (err) {
        // Error notification is already handled by the hook
  ...
  // ✅ Updated single resolve to remove duplicate notifications
  const handleSingleResolve = useCallback(
    async (alertId: number) => {
      if (
        window.confirm(
  ...
  const formatTime = useCallback(
    (timeInput: string | { Time?: string; time?: string } | null) => {
      try {
        ...
        const diffInMinutes = Math.floor(
          Math.abs(diffInMilliseconds) / (1000 * 60),
        );
  ...
  if (loading && alerts.length === 0) {
    return (
      <div className="monitoring-alerts-dashboard">
        <div className="monitoring-alerts-loading">
          <div className="monitoring-alerts-loading-spinner">
  ...
  if (error && alerts.length === 0) {
    return (
      <div className="monitoring-alerts-dashboard">
        <div className="monitoring-alerts-error">
          <div className="monitoring-alerts-error-icon">
            <FaExclamationTriangle />
          </div>
          <div className="monitoring-alerts-error-content">
            <h4>Failed to Load Alerts</h4>
            <p>{error}</p>
            <button
              className="monitoring-alerts-btn monitoring-alerts-btn-primary"
    ...
    <div className="monitoring-alerts-dashboard">
      ...
      <div className="monitoring-alerts-header-section">
        <div className="monitoring-alerts-title-section">
          ...
          <p className="monitoring-alerts-page-subtitle">
        ...
        <div className="monitoring-alerts-header-actions">
          ...
          >
            <FaSync
              className={`monitoring-alerts-btn-icon ${loading ? "spinning" : ""}`}
            />
        ...
        <div className="monitoring-alerts-stats-card">
          ...
          <div className="monitoring-alerts-stats-grid">
            <div className="monitoring-alerts-stat-item">
              <span className="monitoring-alerts-stat-value">
                {stats.total}
              </span>
            ...
            <div className="monitoring-alerts-stat-item monitoring-alerts-stat-new">
              ...
              <span className="monitoring-alerts-stat-label">
                Unacknowledged
              </span>
            </div>
          ...
          <div className="monitoring-alerts-donut-container">
            <div className="monitoring-alerts-donut-chart">
              <div className="monitoring-alerts-donut-center">
                <span className="monitoring-alerts-donut-total">
                  {stats.total}
                </span>
            ...
            <div className="monitoring-alerts-donut-stats">
              <div className="monitoring-alerts-donut-stat monitoring-alerts-high">
                <span className="monitoring-alerts-donut-value">
                  {stats.critical}
                </span>
                <span className="monitoring-alerts-donut-stat-label">
                  Critical
                </span>
              </div>
      ...
      <div className="monitoring-alerts-content-container">
        ...
        {loading && alerts.length > 0 && (
          <div className="monitoring-alerts-updating">
            <FaSpinner className="spinning" />
              ...
              <tbody>
                {filteredAlerts.map((alert) => (
                  <tr key={alert.id} className="monitoring-alerts-table-row">
                    ...
                    <td>
                      <span
                        className={`monitoring-alerts-severity-badge ${getSeverityClass(alert.severity)}`}
                      >
                        {alert.severity}
                      </span>
                    </td>
                    <td>
                      <div className="monitoring-alerts-message">
                        <span className="monitoring-alerts-icon">
                          {getSeverityIcon(alert.severity)}
                        </span>
                        <span className="monitoring-alerts-text">
                          {alert.content}
                        </span>
                      </div>
                    </td>
                    ...
                    <td>
                      <span
                        className={`monitoring-alerts-status-badge ${alert.status}`}
                      >
