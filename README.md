...
- Linux servers for client agents
- Windows serverf for client agents
...
sudo ./client-executable 

On each managed Windows server :

using admin powershell or command prompt run the the particular exe file

client-executable.exe# Server Management Suite (SMS)
...
**Server Management Suite (SMS)** is an all-in-one server management solution designed to simplify and centralize the administration of multiple Linux-based servers. Through a modern web interface, SMS allows system administrators to:
...
- Distribute the client agent to each Linux server to be managed
...
*For detailed documentation, see the `/docs` directory or the original project document.*
...
*For detailed documentation, see the `/docs` directory or the original project document.*servers across institutional networks, data centers, and enterprises.
