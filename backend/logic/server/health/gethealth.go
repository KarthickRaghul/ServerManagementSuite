...
func GetHealth(queries *serverdb.Queries) http.HandlerFunc {
	return func(w http.ResponseWriter, r *http.Request) {
		...
		processedResp := processHealthResponse(clientResp, device.Os)
...
// Process health response based on OS
func processHealthResponse(resp interface{}, osType string) interface{} {
	if strings.ToLower(osType) == "windows" {
		return processWindowsHealthResponse(resp)
	}
...
// Windows-specific health response processing (placeholder for future differences)
func processWindowsHealthResponse(resp interface{}) interface{} {
	// For now, return same format as Linux
	// Future: Windows might have different health metrics or naming conventions
	return resp
}
