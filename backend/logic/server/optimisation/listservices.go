...
func GetServices(queries *serverdb.Queries) http.HandlerFunc {
	return func(w http.ResponseWriter, r *http.Request) {
		...
		clientURL := config.GetClientURL(req.Host, "/client/resource/service")
		...
		processedResp := processServicesResponse(clientResp, device.Os)
...
// Process services response based on OS
func processServicesResponse(resp interface{}, osType string) interface{} {
	if strings.ToLower(osType) == "windows" {
		return processWindowsServicesResponse(resp)
	}
...
// Windows-specific services response processing (placeholder for future differences)
func processWindowsServicesResponse(resp interface{}) interface{} {
	// For now, return same format as Linux
	// Future: Windows services structure differs from Linux (Windows Services vs systemctl)
	return resp
}
