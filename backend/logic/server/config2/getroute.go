...
func HandleGetRouteTable(queries *serverdb.Queries) http.HandlerFunc {
	return func(w http.ResponseWriter, r *http.Request) {
		...
		clientURL := config.GetClientURL(req.Host, "/client/config2/route")
		...
		processedResp := processRouteTableResponse(clientResp, device.Os)
...
// Process route table response based on OS
func processRouteTableResponse(resp interface{}, osType string) interface{} {
	if strings.ToLower(osType) == "windows" {
		return processWindowsRouteTableResponse(resp)
	}
...
// Windows-specific route table response processing (placeholder for future differences)
func processWindowsRouteTableResponse(resp interface{}) interface{} {
	// For now, return same format as Linux
	// Future: Windows route table structure differs from Linux (route print vs ip route)
	return resp
}
