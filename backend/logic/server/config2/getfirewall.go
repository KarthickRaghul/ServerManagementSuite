...
func HandleGetFirewall(queries *serverdb.Queries) http.HandlerFunc {
	return func(w http.ResponseWriter, r *http.Request) {
		...
		clientURL := config.GetClientURL(req.Host, "/client/config2/firewall")
		...
		processedResp := processFirewallResponse(clientResp, device.Os)
...
// Process firewall response based on OS
func processFirewallResponse(resp interface{}, osType string) interface{} {
	if strings.ToLower(osType) == "windows" {
		return processWindowsFirewallResponse(resp)
	}
...
// Windows-specific firewall response processing (placeholder for future differences)
func processWindowsFirewallResponse(resp interface{}) interface{} {
	// For now, return same format as Linux
	// Future: Windows Firewall has different structure than iptables/ufw
	return resp
}
